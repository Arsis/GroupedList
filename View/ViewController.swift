...
public class ViewController: UIViewController {
    ...
    public required init(viewModel:ControllerViewModel, nibName nibNameOrNil: String?, bundle nibBundleOrNil: Bundle?) {
    ...
    fileprivate func itemLongPressDetected(indexPath: IndexPath) {
        if let groupViewModel = self.viewModel?.visibleGroups?[indexPath.section], let itemViewModel = self.viewModel?.itemAtIndexPath(indexPath) {
            let alertController = UIAlertController.init(title: groupViewModel.name, message: itemViewModel.name, preferredStyle: .alert)
...
extension ViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        if let itemViewModel = self.viewModel?.itemAtIndexPath(indexPath), let c = cell as? ControllerTableViewCell {
            c.onLongPressDetected = { [weak self] in
                self?.itemLongPressDetected(indexPath: indexPath)
            }
            c.update(with: itemViewModel)
        }
    }
